...
const updatePost = () => {
  ...
  form.post(route("admin.futurism.upload_images"), {
    onSuccess: () => {
      emit('close');
    //   form.image = null;
      form.type = '';
...
const deleteimage = (id, image_name) => {
    ...
    form.delete(route("admin.futurism.delete.image"), {
    onSuccess: () => {
      emit('close');
      form.id = '';
      form.image_name;
    ...
    <form @submit.prevent="updatePost" class="mt-6 space-y-6">
        <div class="flex flex-wrap gap-4">
            <div v-for="post_image in post.images" class="relative">
                <img :src="`/storage/images/futurism/${post_image.file_name}`" alt="Image 1" class="w-32 h-32 object-cover rounded-lg">
