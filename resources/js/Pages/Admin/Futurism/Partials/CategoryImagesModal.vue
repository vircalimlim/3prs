...
const {showImages, category} = defineProps({
  showImages: {
    type: Boolean,
    required: true
  },
...
const updatePost = () => {
  ...
  form.post(route("admin.futurism.category.upload_images"), {
    onSuccess: () => {
      emit('close');
      form.id = '';
...
const deleteimage = (id, image_name) => {
    ...
    form.delete(route("admin.futurism.category.delete.image"), {
    onSuccess: () => {
      emit('close');
      form.id = '';
      form.image_name;
...
<template>
  <Modal :show="showImages" @close="closeModal">
    <h1 class="text-gray-700 text-2xl font-bold">{{ category.category_name }} Images</h1>
    <form @submit.prevent="updatePost" class="mt-6 space-y-6">
        <div class="flex flex-wrap gap-4">
            <div v-for="post_image in category.images" class="relative">
                <img :src="`/storage/images/futurism/${post_image.image}`" alt="Image 1" class="w-32 h-32 object-cover rounded-lg">
                <button @click.prevent="deleteimage(post_image.id, post_image.image)" class="absolute top-1 right-1 p-1 bg-red-500 text-white rounded-full">
